...
Geprüfte Version: Arbeitsstand nach **2026.0831.01** mit den fünf Etappen vom 02.09.2026 (Fehler die heute wirken, zuverlässig platzieren, Monitore zur Laufzeit, Oberfläche und Ausbau, erweiterte Einstellungen für erfahrene Anwender). Noch nicht als Release veröffentlicht.

## Bestanden

- Release-Build für Windows 11 x64: 0 Fehler, 0 Warnungen (`TreatWarningsAsErrors` in allen Projekten aktiv).
- 621 automatisierte Tests: 621 bestanden, 0 fehlgeschlagen, 0 übersprungen.
...
- **Etappe 5:** `AdvancedSettingsTests` halten fest, dass jede Feinabstimmung mit dem bisherigen festen Verhalten als Voreinstellung startet, im Modell auf ihren Bereich begrenzt wird, den Weg in die Datei und zurück übersteht, dass Werte ausserhalb des Bereichs und ungültige Farben beim Laden abgelehnt werden, dass «Einstellungen zurücksetzen» Erscheinungsbild, Autostart und Rechte behält, dass die Zusatztasten der Zonenkürzel auf die Windows-Kennungen abgebildet werden, dass die Beschriftung dem gewählten Stil folgt, dass der Auffang in der Hauptzone abschaltbar ist und die Toleranz für «eingerastet» beachtet, dass gemerkte Pixel gewinnen, wenn die Zonenpräferenz aus ist, und dass die erweiterten Karten erst mit dem Schalter erscheinen.
...
- Der Schalter «Erweiterte Einstellungen anzeigen» blendet die Karten «Feinabstimmung Ziehen», «Overlay-Stil», «Feinabstimmung Platzieren» und «Schutz und Zeiten» ein; die Karte «Tastenkürzel» zeigt die gewählten Zusatztasten in der Kürzelliste. Alle Karten wurden bei 1480 × 900 und im maximierten Fenster auf 5120 × 2100 in Augenschein genommen, keine Überlappung, kein abgeschnittener Text.

...
.PARAMETER NoRelaunch
    Verhindert den Neustart in Windows PowerShell. Nur fuer den Neustart selbst gedacht, damit er sich
...
if (-not [ZoneManager.FrameProbe]::MakeDpiAware()) {
    # PowerShell 7 traegt die System-DPI-Bewusstheit in seinem Manifest; sie steht damit schon beim Start
    # fest und laesst sich danach nicht mehr aendern -- SetProcessDpiAwarenessContext antwortet mit
    # ERROR_ACCESS_DENIED. Windows PowerShell startet ohne Bewusstheit und laesst sich umstellen, deshalb
    # wird die Messung dort einmal neu gestartet. Ohne diesen Umweg koennte sie unter pwsh nie laufen,
    # und scripts\publish-release.ps1 verlangt pwsh.
    ...
    & $windowsPowerShell -NoProfile -ExecutionPolicy Bypass -File $PSCommandPath `
        -MaximumBorderPixels $MaximumBorderPixels -NoRelaunch
    if ($LASTEXITCODE -ne 0) {
        throw 'Die Messung in Windows PowerShell ist fehlgeschlagen.'
    }

...
$testExecutable = Join-Path $testDirectory 'SaschaWindowZones.exe'
...
try {
    ...
    if (-not (Test-Path -LiteralPath $testExecutable -PathType Leaf)) {
        throw 'Der normale Build hat keine SaschaWindowZones.exe am vorgegebenen Root-Pfad erzeugt.'
    }
    ...
    if ($diagnostic.application -ne 'Sascha Window Zones') {

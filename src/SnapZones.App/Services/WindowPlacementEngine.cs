...
public sealed class WindowPlacementEngine : IWindowPlacementEngine
{
        ...
        bool suppressAlreadyProcessed)
    {
        ...
        if (suppressAlreadyProcessed && processedIdentity is not null)
        {
        ...
        ExecuteInvalidation(() =>
        {
            ...
            if (suppressAlreadyProcessed &&
                processedIdentities.TryGetValue(windowHandle, out var currentProcessedIdentity))
            {
    ...
    private async Task RestoreTriggeredAsync(
        nint windowHandle,
        HandleState state,
        OperationContext context,
        WindowPlacementTrigger trigger,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (explicitZoneId is not null)
        {
            var targetExists = explicitProfileId is not null &&
                environment.Configuration.Layouts.Any(layout => layout.Id == explicitProfileId) &&
        ...
        {
            var activeLayout = environment.Configuration.Layouts.FirstOrDefault(layout =>
                layout.IsActive &&
                (string.Equals(layout.Monitor.StableId, monitor.StableId, StringComparison.OrdinalIgnoreCase) ||
                 string.Equals(layout.Monitor.DeviceName, monitor.StableId, StringComparison.OrdinalIgnoreCase)));
            var zones = environment.Zones
                .Where(zone =>
                    activeLayout is not null &&
                    zone.ProfileId == activeLayout.Id &&
                    zone.MonitorStableId == monitor.StableId)
                .ToArray();

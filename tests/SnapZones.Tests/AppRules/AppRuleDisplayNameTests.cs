...
{
    private const string VersionedPath = @"C:\Users\sascha\AppData\Local\AnthropicClaude\app-1.2.3\claude.exe";
    ...
    public void Without_a_title_the_list_shows_the_file_name_instead_of_the_full_path()
    {
        ...
        Assert.Equal("claude.exe", rule.DisplayName);
        Assert.Equal("claude.exe", rule.ProcessFileName);
        Assert.DoesNotContain("app-1.2.3", rule.DisplayName, StringComparison.Ordinal);
    }

    [Theory]
    [InlineData("claude.exe", "claude.exe")]
    ...
    {
        var rule = Rule("claude.exe");
        var beforeUpdate = new AppWindowIdentity(1, VersionedPath, "Claude", "Chrome_WidgetWin_1");
        var afterUpdate = new AppWindowIdentity(
            2,
            @"C:\Users\sascha\AppData\Local\AnthropicClaude\app-1.3.0\claude.exe",
            "Claude",
            "Chrome_WidgetWin_1");

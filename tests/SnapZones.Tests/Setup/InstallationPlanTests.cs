...
public sealed class InstallationPlanTests
{
    ...
    {
        var plan = InstallationPlan.Create(@"C:\Users\Sascha\Downloads\ZoneManager.exe", ProgramFiles, StartMenu);

        Assert.Equal(InstallationState.NotInstalled, plan.State);
        Assert.Equal(@"C:\Program Files\ZoneManager", plan.TargetDirectory);
        Assert.Equal(@"C:\Program Files\ZoneManager\ZoneManager.exe", plan.TargetPath);
        Assert.Equal(@"C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Sascha's Zone Manager.lnk", plan.ShortcutPath);
    ...
    public void The_entry_in_apps_and_features_can_uninstall_itself_and_offers_nothing_it_cannot_do()
    {
        ...
        Assert.Equal("Sascha's Zone Manager", entry["DisplayName"]);

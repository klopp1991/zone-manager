...
public sealed class AppRuleEditingUiTests
{
    ...
    public void The_add_button_creates_a_visible_second_rule_next_to_an_existing_one()
    {
        WpfThemeHost.Invoke(() =>
        {
            ...
            Assert.Equal(["claude.exe", "notepad.exe"], viewModel.AppRules.Rules.Select(rule => rule.ProcessPath));
    ...
    public void The_process_path_can_be_cleared_in_favour_of_the_title_pattern()
    {
        WpfThemeHost.Invoke(() =>
        {
            var (window, viewModel) = OpenRulesTab(ExistingRule(out var configuration) with { WindowTitlePattern = "Claude" }, configuration);
            var processText = Assert.IsType<TextBox>(window.FindName("AppRuleProcessPathText"));
            ...
            Assert.Equal("Claude", rule.WindowTitlePattern);

...
using ZoneManager.App.Overlays;
...
{
    [Fact]
    public void Monitor_identification_window_shows_the_numbered_custom_name()
    {
        WpfThemeHost.Invoke(() =>
        {
            var monitor = new LiveMonitor(
                new MonitorIdentity("A", "DISPLAY2", "Dell U2723QE"),
                new MonitorWorkArea(-10000, -10000, 1920, 1080),
                96,
                96,
                false);
            ...
            try
            {
                window.ShowFor(monitor, "[2] Rechts");

                Assert.Equal("[2] Rechts", window.Label);
            }
            finally
            {
                window.Close();
